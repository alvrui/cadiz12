#!/bin/bash
# Script para simular una partida básica

cd /media/alvaro/service/project-stack/cadiz12

# Crear script de simulación en examples/
cat > examples/simular.rs << 'RUST_EOF'
...
# Añadir a Cargo.toml si no existe
if ! grep -q "simular" Cargo.toml; then
    cat >> Cargo.toml << 'TOML_EOF'

[[example]]
name = "simular"
path = "examples/simular.rs"
TOML_EOF
fi

# Compilar y ejecutar ejemplo
echo "Compilando y ejecutando simulación..."
cargo build --example simular 2>&1
cargo run --example simular 2>&1
